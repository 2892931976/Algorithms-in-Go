...
type Users struct {
    ...
    Gender        int     `xorm:"tinyint(1)"`       //用户性别
...
// 查找
// 通过Id查找
func (r *Users) GetByMobile(mobile string) (*Users, error) {
    _, err := GetUsersQS().Where("mobile = ?", mobile).Get(r)
    if err != nil {
        log.Printf("查找用户错误: %v", err)
        return nil, err
    }
...
// 查找
// 通过Id查找
func (r *Users) FindAll() ([]*Users, error) {
    roles := make([]*Users, 0)
    err := GetRolesQS().Find(&roles)
    if err != nil {
