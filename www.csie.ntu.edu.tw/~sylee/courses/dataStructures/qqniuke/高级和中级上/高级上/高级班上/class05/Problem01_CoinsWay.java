...
public class Problem01_CoinsWay {
	...
	// arr[0..index]面值自由选择, 组成aim方法数有多少种
	...
	public static int coins2(int[] arr, int aim) {
		...
		// 缓存
		int[][] map = new int[arr.length + 1][aim + 1];// 0 -> 从未被计算过  -1 -> 0
	...
	public static int process2(int[] arr, int index, int aim, int[][] map) {
		...
		if (index == arr.length) {
			...
			int mapValue = 0;
			for (int zhang = 0; arr[index] * zhang <= aim; zhang++) {
				mapValue = map[index + 1][aim - arr[index] * zhang];
				if (mapValue != 0) {
					res += mapValue == -1 ? 0 : mapValue;
				} else {
					res += process2(arr, index + 1, aim - arr[index] * zhang, map);
				}
			}
		}
