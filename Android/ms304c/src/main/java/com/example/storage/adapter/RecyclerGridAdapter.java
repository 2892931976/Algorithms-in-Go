...
import com.example.storage.widget.RecyclerExtras.OnItemClickListener;
import com.example.storage.widget.RecyclerExtras.OnItemLongClickListener;
...
public class RecyclerGridAdapter extends RecyclerView.Adapter<ViewHolder> implements
    ...
    private ArrayList<GridInfo> mGoodsArray;

    public RecyclerGridAdapter(Context context, ArrayList<GridInfo> goodsArray) {
        mContext = context;
    ...
    public void onBindViewHolder(ViewHolder vh, final int position) {
        ...
        if (mGoodsArray.get(position).getImg() != "") {
            if (mGoodsArray.get(position).getImg().contains("jpg")) {
                getImageByUrl.setImage(holder.iv_pic, mGoodsArray.get(position).getImg());
            }
        }

//        holder.iv_pic.setImageResource(mGoodsArray.get(position).pic_id);
        holder.tv_title.setText(mGoodsArray.get(position).getGridName());
        // 列表项的点击事件需要自己实现
    ...
    public void onItemClick(View view, int position) {
        String desc = String.format("您点击了第%d项，栏目名称是%s", position + 1,
                mGoodsArray.get(position).getGridName());
