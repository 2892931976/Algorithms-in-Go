...
import com.example.msmk.socker_server.entity.MessageID;
import com.example.msmk.socker_server.entity.message.CallbackResponse;
import com.example.msmk.socker_server.entity.message.DelayResponse;
import com.example.msmk.socker_server.entity.message.ServerHeartBeat;
import com.example.msmk.socker_server.entity.message.TestResponse;
import com.example.msmk.socker_server.entity.message.base.SuperClient;
import com.example.msmk.socker_server.entity.message.base.SuperResponse;
import com.example.msmk.socker_server.iowork.IWriter;
    ...
    public void handReceiveMsg(String receiver) {
        ...
        switch (id) {
            case MessageID.CALLBACK_MSG: //回调消息
                ...
                ((CallbackResponse) superResponse).setFrom("I im callback server");
                break;

            case MessageID.TEST_MSG: //测试消息
                superResponse =new TestResponse();
                superResponse.setMsgId(MessageID.TEST_MSG);
                ((TestResponse) superResponse).setFrom("I im test server");
                break;
            case MessageID.HEARTBEAT: //心跳包
                superResponse = new ServerHeartBeat();
                ((ServerHeartBeat) superResponse).setFrom("I im HearBeat server");
            ...
            case MessageID.DELAY_MSG: //延时消息
                ...
                ((DelayResponse) superResponse).setFrom("I im Delay server");
                try {
                    Thread.sleep(1000*5);
