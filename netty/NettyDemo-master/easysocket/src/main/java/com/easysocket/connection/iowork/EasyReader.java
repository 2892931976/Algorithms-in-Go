...
public class EasyReader implements IReader<EasySocketOptions> {
    ...
    public void read() {
        ...
        try {
            ...
            byte[] bs = originalData.getHeaderData();
            int abs = 0;
            for (int i = bs.length - 1; i >= 0; i--) {
                abs += bs[i] * Math.pow(0xFF, bs.length - i - 1);
            }
//            String headerStr = new String(originalData.getHeaderData());
            LogUtil.d("接收的原始头部=" + abs);
            ...
            //将读取到一个完整数据发布出去
            LogUtil.d("接收的原始数据=" + originalData.getBodyString());
            LogUtil.d("接收的原始length=" + originalData.getBodyString().length());
            actionDispatch.dispatchAction(IOAction.ACTION_READ_COMPLETE, originalData);
        } catch (Exception e) {
            e.printStackTrace();
        }
